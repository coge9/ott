...
/**
 * Created by Ivan_Karnilau on 9/27/2016.
 */
public class TC16103_CustomRuleIos2Latest extends BaseAuthFlowTest {
    ...
    private void customRuleIOS2Latest(String brand) {
        ...
        //Step 1
        programsToAdd = rokuBackEndLayer.getPublishedPrograms(2);
        custom = (Shelf) rokuBackEndLayer.getRulesStrategyForConcerto(ShelfType.SHELF, rokuBackEndLayer.getPublishedPrograms(2), 2);

        //Step 2
        custom = (Shelf) rokuBackEndLayer.createModule(custom);

        //Step 3
        DraftModuleTab draftModuleTab = new DraftModuleTab(webDriver, aid);
        draftModuleTab.checkLatestEpisodesByName(programsToAdd, 2).clickSave();
        custom = draftModuleTab.getShelfInfo(custom);
        ...
        pageForm = editPage.updateFields(Arrays.<Module>asList(custom));
        editPage.save();

        softAssert.assertTrue(draftModuleTab.elementPublishBlock().isPublishTabPresent(PublishInstance.DEV),
                "The Development instance tab is not present", "The Development tab is shown");


        //Step 6
        draftModuleTab.elementPublishBlock().publishByTabName();
        softAssert.assertTrue(draftModuleTab.isStatusMessageShown(), "The status message is not shown after publishing",
                "The status message is shown after publishing", webDriver);
        softAssert.assertFalse(draftModuleTab.isErrorMessagePresent(),
                "The error message is present after publishing with message" + mainRokuAdminPage.getErrorMessage(),
        ...
        //Get Expected result
        CollectionJson expectedCollectionJson = new CollectionJson(custom);
    ...
    public void cleanUpContent() {
        try {
            ...
            rokuBackEndLayer.deleteModule(custom.getTitle());
        } catch (Throwable e) {
