 ...
 */
public class TC15020_SaveAndPublishEvent extends BaseAuthFlowTest {
    ...
    public void savePublishEvent(final String brand) {
        ...
        //Step 2
        ContentTypePage contentTypePage = (ContentTypePage) mainRokuAdminPage.openPage(event.getPage(), brand);
        ...
        //Step 5-7
        contentTypePage = rokuBackEndLayer.createContentType(event);
        ...
        softAssert.assertEquals(PublishState.NOT_PUBSLISHED.getStateValue(), contentPage.getNodePublishState(event.getTitle()), "The node state is Published", "The node state is Not Published", webDriver);

        //Step 9
        contentTypePage = rokuBackEndLayer.editContentType(event);
        ...
        softAssert.assertEquals(PublishState.PUBLISHED.getStateValue(), contentPage.getNodePublishState(event.getTitle()), "The node state is not Published"
        ...
        //Step 17
        contentPage = contentTypePage.cancel();
        softAssert.assertEquals(PublishState.NOT_PUBSLISHED.getStateValue(), contentPage.getNodePublishState(event.getTitle()), "The node state is Published after draft save"
        ...
        //Step 21
        contentPage = contentTypePage.cancel();
        softAssert.assertEquals(PublishState.PUBLISHED.getStateValue(), contentPage.getNodePublishState(event.getTitle()), "The node state is not Published after draft save"
