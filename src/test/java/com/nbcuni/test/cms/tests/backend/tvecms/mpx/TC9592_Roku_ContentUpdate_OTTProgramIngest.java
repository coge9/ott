 ...
 *
 */
public class TC9592_Roku_ContentUpdate_OTTProgramIngest extends BaseAuthFlowTest {
    ...
    public void mpxShowContentUpdate() throws InterruptedException {
        Utilities.logInfoMessage("Check that OTT Program is updated by whole metadata after ingest from MPX Programs on roku " + brand);
        //Step 1
        ...
        //Step 3
        if (contentPage.searchByTitle(config.getAvailableShow()).searchByType(ContentType.TVE_PROGRAM).apply()) {
            episodeForFullUpdate = contentPage.getContentFullTitle(config.getAvailableShow());

        } else {
            contentPage.searchByTitle(MPX_AUTOMATION_PROGRAM).searchByType(ContentType.TVE_PROGRAM).apply();
            episodeForFullUpdate = contentPage.getContentFullTitle(MPX_AUTOMATION_PROGRAM);
        }
        if (episodeForFullUpdate == null) {
            Assertion.fail(
                    "There is no TV Episode with title contains: " + MPX_AUTOMATION_PROGRAM + " or " + config.getAvailableShow());
        }
        ...
        final String newTitle = MPX_AUTOMATION_PROGRAM + SimpleUtils.getRandomString(2);
        ...
        softAssert.assertEquals(actualMPXData, expectedMPXData,
                "OTT Program was not properly updated in Roku CMS after update on MPX thePlatform.");
        Utilities.logInfoMessage("OTT Program was properly updated in Roku CMS after update on MPX thePlatform.");
