...
public class TC10810_PublishPageWithOneGeneratedHeaderHeaderWithPageTitle
        extends BaseAuthFlowTest {


    /**
     * TC10810 - Roku CMS: Publishing: Header: Publish page with one generated header (Header With Page Title)
     *
    ...
    public void createPage() {
        ...
        headerGeneration.generateOttPageHeaders(pageTitle, headerStyle,
                mpvdName);
        headerUrl = headerGeneration.getOttPageHeaders(pageTitle)
                .get(mpvdId);
    }
    ...
    public void noHeaderPublishingIfNoHeader(final String brand) {
        ...
        RokuHeaderJson rokuHeaderJson = new RokuHeaderJson(
                SimpleUtils.getMd5(headerUrl), CmsPlatforms.getAppIdBYAppName(appName, brand), mpvdId, pageAlias,
                headerUrl);
        softAssert.assertTrue(publishedHeaderOperations.size() == 1,
                "Number of published headers are differ from 1");
        softAssert.assertTrue(
                publishedHeaderOperations.get(0).equals(rokuHeaderJson),
                "Data in JSON differ from expected");
