 ...
 */
public class TC15031_SaveAndPublishingFailForCollectionGroup extends SetupAmazonEndpoint {
    /**
     ...
     * Verify: node is created but node is NOT published
     * <p/>
     * 10.Go to Collection list. Check node's state
     * Verify: state is "Not Published"
     * <p/>
     ...
     * Verify: publishing dialog is displayed
     * <p/>
     * 13.Choose API Service instance. Click on "Publish" button
     * Verify: node is saved but node is NOT published
     * <p/>
     * 14.Go to Collection list.Check node's state
     * Verify: state is "Not Published"
     */
    ...
    public void savePublishFailCollectionGroup(final String brand) {
        //Step 1
        ...
        //Step 4-5
        collectionPage.publish(endpoint);
        ...
        softAssert.assertEquals(PublishState.NOT_PUBSLISHED.getStateValue(), collectionsContentPage.getNodePublishState(collection.getTitle()), "The node state is Published"
                , "The node state is not Published", webDriver);

        //Step 7
        collectionPage = collectionsContentPage.openEditCollectionGroupPage(collection);
        collectionPage.publish(endpoint);
        ...
        //Step 10
        mainRokuAdminPage.openPage(CollectionsContentPage.class, brand);
        softAssert.assertEquals(PublishState.NOT_PUBSLISHED.getStateValue(), collectionsContentPage.getNodePublishState(collection.getTitle()), "The node state is Published"
                , "The node state is not Published", webDriver);

        //Step 11
        collectionPage = collectionsContentPage.openEditCollectionGroupPage(collection);
        collectionPage.publish(ApiInstanceConstant.AMAZON_API);
        ...
        //Step 14
        mainRokuAdminPage.openPage(CollectionsContentPage.class, brand);
        softAssert.assertEquals(PublishState.PUBLISHED.getStateValue(), collectionsContentPage.getNodePublishState(collection.getTitle()), "The node state is not Published"
    ...
    public void deleteCollectionGroupTC15031() {
        try {
