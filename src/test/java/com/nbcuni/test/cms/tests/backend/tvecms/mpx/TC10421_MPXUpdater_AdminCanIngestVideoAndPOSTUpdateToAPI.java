...
public class TC10421_MPXUpdater_AdminCanIngestVideoAndPOSTUpdateToAPI extends BaseAuthFlowTest {
    ...
    @Test(groups = {"roku_ingest_debug"}, alwaysRun = true, enabled = true)
    @Parameters({"brand"})
    public void specifyMPXIDAndUpdateDirectly(final String brandInUse) {
        brand = brandInUse;
        Utilities.logInfoMessage("Check that Admin can Specify MPX ID of a Video,Update it directly and POST to API" + brand);
        ...
        //Step 3
        if (contentPage.searchByTitle(MPX_AUTOMATION_EPISODE).searchByType(ContentType.TVE_VIDEO).apply()) {
            episodeForFullUpdate = contentPage.getContentFullTitle(MPX_AUTOMATION_EPISODE);

        } else {
            contentPage.searchByTitle(config.getAvailableEpisode()).searchByType(ContentType.TVE_VIDEO).apply();
        ...
        final String newTitle = MPX_AUTOMATION_EPISODE + SimpleUtils.getRandomString(10);
        ...
        final Map<String, String> actualMPXData;// = editPage.collectMPXData();
        actualMPXData = editPage.collectMPXData();
        RokuVideoJson videoExpectedObjects = contentPage.getVideoObject(newTitle, brand);
        RokuVideoJson videoActualObjects = requestHelper.getSingleParsedResponse(url, SerialApiPublishingTypes.VIDEO);

        softAssert.assertEquals(videoExpectedObjects, videoActualObjects, "The Video object in the CMS is not matched with those that send",
                "The Video object in the CMS is matched with those that send", webDriver);
