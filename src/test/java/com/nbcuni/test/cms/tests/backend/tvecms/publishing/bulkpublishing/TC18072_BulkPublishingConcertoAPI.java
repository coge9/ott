 ...
 */
public class TC18072_BulkPublishingConcertoAPI extends BaseAuthFlowTest {
    ...
    public void checkThatNodePublishesToConcertoOnly(final String brand) {
        ...
        //Step 4
        instancePage.publishToInstance(instancePage.getConcertoInstanceName().get(0));
        ...
        for (LocalApiJson localApiJson : localApiJsons) {
            softAssert.assertTrue(StringUtils.containsIgnoreCase(localApiJson.getEndpoint(), PublishInstance.CONCERTO.getName()),
                    "The bulk publishing was happen for not Concerto API only, but to: " + localApiJson.getEndpoint(),
                    "The bulk publishing was happen for Concerto API only");
        }
