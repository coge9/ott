 ...
 */
public class TC15029_SaveAndPublishingFailForPost extends SetupAmazonEndpoint {
    /**
    ...
    public void savePublishFailPost(final String brand) {
        //Step 1
        ...
        softAssert.assertEquals(PublishState.NOT_PUBSLISHED.getStateValue(), contentPage.getNodePublishState(post.getTitle()), "The node state is Published"
                , "The node state is not Published", webDriver);

        //Step 7
        rokuBackEndLayer.editContentType(post);
        contentTypePage.publish(endpoint);
