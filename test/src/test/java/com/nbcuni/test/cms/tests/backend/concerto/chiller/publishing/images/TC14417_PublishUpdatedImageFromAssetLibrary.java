 ...
 * <p>
 * Step 4. Update fields for the selected image and click button 'publish'
 * Verify: The metadata has saved and POST request is send to Amazon API
 *
 * Step 5. Click button 'publish'
 * Verify:    The POST request is send to API. Response message is 'success'
 * <p>
 * Step 6. Check scheme of JSON within POST request and values
...
public class TC14417_PublishUpdatedImageFromAssetLibrary extends BaseAuthFlowTest {
    private FilesMetadata expectedFilesMetadata;
    private FilesMetadata updatedFilesMetadata;

    @BeforeMethod(alwaysRun = true)
    public void initFileMetadata() {
        expectedFilesMetadata = FilesMetadataCreationStrategy.createFilesMetadata();
        updatedFilesMetadata = FilesMetadataCreationStrategy.createFilesMetadata();
    }
    ...
    public void test(String brand) {
        ...
        //update metadata
        editMultipleFilesPage.enterFilesMetadata(updatedFilesMetadata);
        editMultipleFilesPage.save();

        softAssert.assertTrue(editMultipleFilesPage.isStatusMessageShown(), "Status message is not presented",
                "Status message is presented", webDriver);
        //Get Expected result
        ImagesJson expectedImage = editMultipleFilesPage.updateMetadataForPublishing(updatedFilesMetadata);
        //republish image
        editMultipleFilesPage.publish();
