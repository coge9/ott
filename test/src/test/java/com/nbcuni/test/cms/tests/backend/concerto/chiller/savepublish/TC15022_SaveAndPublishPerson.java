 ...
 */
public class TC15022_SaveAndPublishPerson extends BaseAuthFlowTest {
    ...
    public void savePublishPerson(final String brand) {
        ...
        //Step 2
        ContentTypePage contentTypePage = (ContentTypePage) mainRokuAdminPage.openPage(person.getPage(), brand);
        ...
        softAssert.assertContains(webDriver.getTitle(), MetadataPage.PAGE_TITLE, "Metadata page is not opened",
                "Metadata page is opened", webDriver);

        //Step 5-7
        contentTypePage = rokuBackEndLayer.createContentType(person);
        ...
        softAssert.assertEquals(PublishState.NOT_PUBSLISHED.getStateValue(), contentPage.getNodePublishState(person.getTitle()), "The node state is Published", "The node state is Not Published", webDriver);

        //Step 9
        contentTypePage = rokuBackEndLayer.editContentType(person);
        ...
        contentPage.searchByTitle(person.getTitle()).apply();
        softAssert.assertEquals(PublishState.PUBLISHED.getStateValue(), contentPage.getNodePublishState(person.getTitle()), "The node state is not Published"
        ...
        //Step 17
        mainRokuAdminPage.openPage(MetadataPage.class, brand);
        softAssert.assertEquals(PublishState.NOT_PUBSLISHED.getStateValue(), contentPage.getNodePublishState(person.getTitle()), "The node state is Published after draft save"
        ...
        //Step 21
        mainRokuAdminPage.openPage(MetadataPage.class, brand);
        softAssert.assertEquals(PublishState.PUBLISHED.getStateValue(), contentPage.getNodePublishState(person.getTitle())
    ...
    public void deleteRoleTC15020() {
        try {
            ...
            Utilities.logSevereMessageThenFail("Couldn't delete the content");
        }
    }
}
